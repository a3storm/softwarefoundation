...
Proof.
  split.
    ...
    Case "<-".
      generalize dependent b. induction e; simpl. 
        try (intros; subst). apply E_BTrue.
        try (intros; subst). apply E_BFalse.
        intros. subst. constructor. apply aeval_iff_aevalR. reflexivity.
        apply aeval_iff_aevalR. reflexivity.  
        intros. subst. constructor. apply aeval_iff_aevalR. reflexivity. 
        apply aeval_iff_aevalR. reflexivity.
        intros. subst. constructor. apply IHe. reflexivity. 
        intros. subst. constructor. apply IHe1. reflexivity. 
...
Proof.
  intros. 
  unfold update. 
  induction eq_id_dec; subst; reflexivity. 
Qed. 
(** [] *)
...
Proof.
  ...
  induction eq_id_dec;
  destruct eq_id_dec;
  try (apply ex_falso_quodlibet; apply H; subst;  reflexivity);
  reflexivity.
Qed. 
(** [] *)
...
Proof. 
intros. 
  inversion H1. subst. clear H1. simpl. apply update_eq. 
Qed. 
...
Proof.
  ...
  rewrite -> H1 in H. inversion H. subst. apply IHcontra2.
  apply Heqloopdef. 
Qed. 
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
  ...
  | NW_Ass  : forall x X, no_whilesR (x ::= X)
  | NW_Seq  : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
  | NW_Iff  : forall b ct cf, no_whilesR ct -> no_whilesR cf -> no_whilesR (IFB b THEN ct ELSE cf FI)
  .
...
Proof.
  split. 
    induction c; simpl. constructor.
    constructor. constructor. apply IHc1. apply andb_true_iff in H. apply H.
    apply IHc2. apply andb_true_iff in H. apply H.
    intros. apply andb_true_iff in H. constructor. apply IHc1. apply H. apply IHc2. apply H.
    intros. inversion H.
    intros. induction H; try reflexivity. simpl. apply andb_true_iff. rewrite IHno_whilesR1. rewrite IHno_whilesR2. split; reflexivity. 
...
Proof.
  intros. 
  generalize dependent st. induction H; try intro.
  exists st. constructor.
  exists (update st x (aeval st X0)). constructor. reflexivity.
  destruct (IHno_whilesR1 st). destruct (IHno_whilesR2 x). exists x0. apply E_Seq with x. apply H1. apply H2.
  remember (beval st b) as condition. destruct condition.
  
      destruct (IHno_whilesR1 st). exists x. apply E_IfTrue. symmetry. apply Heqcondition. apply H1.
      destruct (IHno_whilesR2 st). exists x. apply E_IfFalse. symmetry. apply Heqcondition. apply H1.
Qed.
(* FILL IN HERE *)
(** [] *)
